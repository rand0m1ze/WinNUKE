...
namespace LOIC
{
	public partial class frmMain : Form
	{
		...
		{
			if((cmdAttack.Text == "HERE COMES THA B00M" && toggle == true) || (toggle == false && on == true))
			{
				try
				{
			...
			{
				cmdAttack.Text = "HERE COMES THA B00M";
				if(xxp != null)
				{
					for (int a = 0; a < xxp.Length; a++)
					{
		...
		private void LockOnIP(bool silent)
		{
			if(txtTargetIP.Text.Length == 0)
			{
				if(silent) return;
				new frmWtf().Show();
				MessageBox.Show("Enter IP address to destroy");
				return;
			}
		...
		private void LockOnURL(bool silent)
		{
			...
			catch
			{
				if(silent) return;
				new frmWtf().Show();
				MessageBox.Show("URL did not resolve to IP...");
				return;
			}
		}
			...
			foreach (string param in pars)
			{
				...
				else
				{
					...
					else if(sp[0].ToLower() == "default")
					{
						 ...
						 txtSubsite.Text = "/";
						 txtData.Text = "Here comes the B00M";
						 txtPort.Text = "80";
		...
		{
			this.Text = String.Format("{0} | Hold the Sk1D V1.0", Application.ProductName, Application.ProductVersion);
		}
